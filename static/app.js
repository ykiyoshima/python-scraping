...
function handleStartButtonClick() {
    ...
    recognition.onresult = event => {
      ...
      if (!isFinal) {
        ...
        } else if (text.indexOf('セール') !== -1) {
            let info = text.split('の')[0];
            // セールだったら、API経由でおすすめ記事を取得する.
            showDTMSale(info);
      } else {
        // セール以外はわからないよ〜.
        let synthes = new SpeechSynthesisUtterance('ごめんなさい、ニュースとセール以外はわかりません');
        synthes.lang = "ja-JP";
...
function showDTMSale(info) {

    api(`/api/dtm_sale/${info}`).then(response => {
        let { content, link } = JSON.parse(response);
        console.log(content);
...
function startIntro() {
    ...
    return new Promise((resolve, reject) => {
        ...
        let texts = "「○○のセールを教えて」と聞いてみてください。例）ドラムのセールを教えて".split('');
